...
# Saskatchewan Canadian Prairie Canola (CAN) — DDcentEVI 4.91
# =============================================================================
...
#   1 105 CROP CAN   → 1 {seed_doy}     CROP CAN
...
# matching the CAN calibration crop.100 — not optimized. BASETEMP=0 here matches
# the calibration and the Miller et al. (2001) / Seeding_Date_App canola GDD targets
# (canola flowering window 1230-1565 GDD0; swath-ready ~1900 GDD0).
# =============================================================================
...
config_dates <- list(
  ...
  sites_dir     = "/home/preston/Projects/Daycent/Crop_Optimization/RM/Models/Canola/qrts",
  results_dir   = "/home/preston/Projects/Daycent/Crop_Optimization/Models/Canola/results",
  schedule_name = "Crop_Opt.sch",
  weather_name  = "weather.wth",
  
  # GDD / phenology values — MUST match the CAN calibration crop.100 (BASETEMP=0)
  # Consistency note: the DayCent CAN calibration fixes BASETEMP=0.0, so GDD
  # accumulation here uses base 0C to stay internally consistent with the
  # Seeding_Date_App canola work (flowering window 1230-1565 GDD0; swath-ready
  # ~1900 GDD0 at base 0C). This replaces the earlier base-5C parameterization.
  #
  # TMPGERM: canola germinates at ~2C (Sask Agriculture); reliable rapid
  #          germination above 5C. Use 2.0 air-temp proxy (early-seeded window).
  # TMPKILL: canola seedlings killed at -3 to -5C; -4C conservative threshold.
  # DDBASE:  ~1900 GDD0 to swath/maturity (Seeding_Date_App canola calibration).
  # BASETEMP2: canola yield sensitive to temps >31C in July (Kutcher et al. 2010);
  #            29C ceiling — lower than wheat/barley to reflect heat sensitivity.
  TMPGERM   = 2.0,    # min germination temperature (°C) — air temp proxy
  TMPKILL   = -4.0,   # killing frost temperature (°C) — canola sensitive to frost
  DDBASE    = 1900.0, # GDD to maturity/swath at BASETEMP=0 (Seeding_Date_App canola)
  BASETEMP  = 0.0,    # base temperature for GDD accumulation (°C) — matches calibration
  BASETEMP2 = 29.0,   # ceiling temperature (°C) — canola yield sensitive to heat >31C
  ...
  # Harvest constraints — canola harvested later than cereals
  earliest_harvest = 220,  # DOY 220 = August 8 — canola matures later than cereals
  latest_harvest   = 290   # DOY 290 = October 17 — hard cutoff (frost risk)
)
...
update_schedule_file <- function(sch_path, seed_doy, harvest_doy) {
  ...
  lines <- replace_doy(lines, "CROP CAN",   seed_doy)
