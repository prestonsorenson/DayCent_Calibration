...
# Saskatchewan Canadian Prairie Oats (OAT1) — DDcentEVI 4.91
# =============================================================================
...
#   1 105 CROP OAT1  → 1 {seed_doy}     CROP OAT1
...
# matching the OAT1 calibration crop.100 — not optimized. BASETEMP=0 here matches
# the calibration and the Miller et al. (2001) / Seeding_Date_App GDD targets.
# =============================================================================
...
config_dates <- list(
  ...
  sites_dir     = "/home/preston/Projects/Daycent/Crop_Optimization/RM/Models/Oats/qrts",
  results_dir   = "/home/preston/Projects/Daycent/Crop_Optimization/Models/Oats/results",
  schedule_name = "Crop_Opt.sch",
  weather_name  = "weather.wth",
  
  # GDD / phenology values — MUST match the OAT1 calibration crop.100 (BASETEMP=0)
  # Consistency note: the DayCent OAT1 calibration fixes BASETEMP=0.0, so GDD
  # accumulation here uses base 0C to stay internally consistent. The Miller
  # et al. (2001) GDD stage values used in the Seeding_Date_App are likewise
  # base 0C. Oats require ~1200 GDD at base 5C (Manitoba Agriculture); at base 0C
  # over a ~100-day SK season ≈ 1600 GDD0. Matches daycent_calibration_oats_hpc.R.
  #
  # TMPGERM: air-temp proxy; oats germinate at 1-2C like barley (cold tolerant).
  #          Use 1.0 to match the calibration.
  # TMPKILL: -5C killing frost (oats similar frost tolerance to spring cereals).
  TMPGERM   = 1.0,    # min germination temperature (°C) — air temp proxy
  TMPKILL   = -5.0,   # killing frost temperature (°C) — similar to wheat/barley
  DDBASE    = 1600.0, # GDD to maturity at BASETEMP=0 — oats (~1200 at base 5C)
  ...
  # Harvest constraints — oats harvest similar to barley
  earliest_harvest = 200,  # DOY 200 = July 19
...
update_schedule_file <- function(sch_path, seed_doy, harvest_doy) {
  ...
  lines <- replace_doy(lines, "CROP OAT1",  seed_doy)
