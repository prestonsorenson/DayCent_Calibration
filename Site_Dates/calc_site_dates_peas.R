...
# Saskatchewan Canadian Prairie Field Peas (PEA) — DDcentEVI 4.91
# =============================================================================
...
#   1 105 CROP PEA   → 1 {seed_doy}     CROP PEA
...
# matching the PEA calibration crop.100 — not optimized. BASETEMP=0 here matches
# the calibration and the Seeding_Date_App GDD targets.
# =============================================================================
...
config_dates <- list(
  ...
  sites_dir     = "/home/preston/Projects/Daycent/Crop_Optimization/RM/Models/Peas/qrts",
  results_dir   = "/home/preston/Projects/Daycent/Crop_Optimization/Models/Peas/results",
  schedule_name = "Crop_Opt.sch",
  weather_name  = "weather.wth",
  
  # GDD / phenology values — MUST match the PEA calibration crop.100 (BASETEMP=0)
  # Consistency note: the DayCent PEA calibration fixes BASETEMP=0.0, so GDD
  # accumulation here uses base 0C to stay internally consistent. Field peas are
  # among the earliest-maturing SK crops (~85-95 days); DDBASE=1100 at base 0C
  # matches daycent_calibration_peas_hpc.R (the lowest GDD target of all crops,
  # consistent with peas' short season).
  #
  # TMPGERM: air-temp proxy; field peas germinate at ~2-4C (cold tolerant,
  #          among the earliest-seeded crops). Use 2.0 to match the calibration.
  # TMPKILL: -4C killing frost; peas tolerate light frost early (growing point
  #          below soil) but are sensitive later (Saskatchewan Pulse Growers).
  # BASETEMP2: 25C ceiling — field pea yield declines when Tmax >25C during
  #            flowering (Siddiqui et al. 2021); lower than cereals.
  TMPGERM   = 2.0,    # min germination temperature (°C) — air temp proxy
  TMPKILL   = -4.0,   # killing frost temperature (°C) — tolerant early, sensitive later
  DDBASE    = 1100.0, # GDD to maturity at BASETEMP=0 — peas earliest-maturing crop
  BASETEMP  = 0.0,    # base temperature for GDD accumulation (°C) — matches calibration
  BASETEMP2 = 25.0,   # ceiling temperature (°C) — peas sensitive to heat >25C at flowering
  ...
  # Harvest constraints — peas mature earlier than cereals
  earliest_harvest = 195,  # DOY 195 = July 14
  latest_harvest   = 275   # DOY 275 = October 2 — hard cutoff
)
...
update_schedule_file <- function(sch_path, seed_doy, harvest_doy) {
  ...
  lines <- replace_doy(lines, "CROP PEA",   seed_doy)
