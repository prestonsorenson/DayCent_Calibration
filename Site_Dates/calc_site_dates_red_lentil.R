...
# Saskatchewan Canadian Prairie Red Lentil (RLENT) — DDcentEVI 4.91
# =============================================================================
...
#   1 105 CROP RLENT → 1 {seed_doy}     CROP RLENT
...
# matching the red lentil calibration crop.100 (RLENT block) — not optimized.
...
config_dates <- list(
  ...
  sites_dir     = "//home/preston/Projects/Daycent/Crop_Optimization/RM/Models/Red_Lentil/qrts",
  results_dir   = "/home/preston/Projects/Daycent/Crop_Optimization/Models/Red_Lentil/results",
  schedule_name = "Crop_Opt.sch",
  weather_name  = "weather.wth",
  
  # GDD / phenology values — MUST match the red lentil calibration crop.100 (BASETEMP=0)
  # Consistency note: the DayCent LENT calibration fixes BASETEMP=0.0, so GDD
  # accumulation here uses base 0C to stay internally consistent. Red lentil matures
  # in ~85-95 days in SK (slightly earlier than green lentil); at base 0C ≈ 1350 GDD0.
  # Matches daycent_calibration_red_lentil_hpc.R.
  #
  # TMPGERM: air-temp proxy; lentil germinates at ~2-4C (cold tolerant, seeded
  #          early). Use 2.0 to match the calibration.
  # TMPKILL: -5C killing frost; lentil hypogeal germination, regrows from scale
  #          nodes, survives -4 to -6C (Saskatchewan Pulse Growers).
  # BASETEMP2: 27C ceiling — red lentil heat-sensitive at flowering; early seeding
  #            avoids flower blast from high temperatures (Sask Agriculture).
  TMPGERM   = 2.0,    # min germination temperature (°C) — air temp proxy
  TMPKILL   = -5.0,   # killing frost temperature (°C) — survives -4 to -6C
  DDBASE    = 1350.0, # GDD to maturity at BASETEMP=0 — red lentil ~85-95 days
  BASETEMP  = 0.0,    # base temperature for GDD accumulation (°C) — matches calibration
  BASETEMP2 = 27.0,   # ceiling temperature (°C) — heat sensitive at flowering
  ...
  # Harvest constraints — red lentil matures earlier than green lentil
  earliest_harvest = 195,  # DOY 195 = July 14
  latest_harvest   = 270   # DOY 270 = September 27 — hard cutoff (frost risk)
)
...
update_schedule_file <- function(sch_path, seed_doy, harvest_doy) {
  ...
  lines <- replace_doy(lines, "CROP RLENT", seed_doy)
