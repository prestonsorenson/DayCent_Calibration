...
# Saskatchewan Canadian Hard Red Spring Wheat (SW0) — DDcentEVI 4.91
# =============================================================================
...
#   1 105 CROP SW0   → 1 {seed_doy}     CROP SW0
#   1 105 PLTM       → 1 {seed_doy}     PLTM
#   1 273 LAST       → 1 {harvest_doy}  LAST
#   1 273 HARV G     → 1 {harvest_doy}  HARV G
#
# Phenology values (TMPGERM, DDBASE, BASETEMP) are now FIXED literature values
# matching the SW0 calibration crop.100 — not optimized. BASETEMP=0 here matches
# the calibration and the Miller et al. (2001) GDD targets from the Seeding_Date_App.
# =============================================================================
...
config_dates <- list(
  ...
  sites_dir     = "/home/preston/Projects/Daycent/Crop_Optimization/RM/Models/Wheat/qrts",
  results_dir   = "/home/preston/Projects/Daycent/Crop_Optimization/Models/Wheat/results",
  schedule_name = "Crop_Opt.sch",
  weather_name  = "weather.wth",
  
  # GDD / phenology values — MUST match the calibration crop.100 (BASETEMP=0)
  # Consistency note: the DayCent SW0 calibration fixes BASETEMP=0.0, so GDD
  # accumulation here uses base 0C to stay internally consistent. The Miller
  # et al. (2001, MSU Ext MT200103AG) GDD stage values used in the Seeding_Date_App
  # are likewise base 0C. At base 0C, HRSW reaches physiological maturity / swath
  # at ~1700 GDD0 (equivalent to ~1200 at base 5C over a ~100-day season).
  #
  # TMPGERM: air-temperature proxy for germination threshold; HRSW germinates
  #          from ~2-5C. Use 2.0 to match the calibration TMPGERM and the
  #          early-seeding Prairie window (Gan et al., Swift Current).
  # TMPKILL: -6C killing frost (HRSW slightly hardier than barley).
  TMPGERM   = 2.0,    # min germination temperature (°C) — air temp proxy
  TMPKILL   = -6.0,   # killing frost temperature (°C)
  DDBASE    = 1700.0, # GDD to maturity at BASETEMP=0 (Miller et al. 2001; ~1200 at base 5C)
  BASETEMP  = 0.0,    # base temperature for GDD accumulation (°C) — matches calibration
  BASETEMP2 = 30.0,   # ceiling temperature for GDD accumulation (°C)
  ...
  # Harvest constraints
  earliest_harvest = 200,  # DOY 200 = July 19 — never harvest before this
  latest_harvest   = 280   # DOY 280 = October 7 — hard cutoff
)
...
update_schedule_file <- function(sch_path, seed_doy, harvest_doy) {
  ...
  lines <- replace_doy(lines, "CROP SW0",   seed_doy)
