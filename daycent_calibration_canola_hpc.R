# =============================================================================
# DayCent CAN Canola Calibration — RM Level
...
config <- list(
  ...
  template_dir   = "/home/prs352/Daycent/Crop_Optimization/Models/Canola/Copy_Folder",
  sites_dir      = "/home/prs352/Daycent/Crop_Optimization/Models/Canola/qrts",
  results_dir    = "/home/prs352/Daycent/Crop_Optimization/Models/Canola/results",
  obs_data_path  = "/home/prs352/Daycent/Crop_Optimization/Models/input_data/canola_c_gmsq.csv",
...
write_crop100 <- function(params, template_path, output_path, crop_name = "CAN") {
...
run_all_rms <- function(params, config) {
  ...
  results <- tryCatch({
    withTimeout(
      ...
      timeout   = 300,   # 5 min — kill evaluation if cluster hangs
      onTimeout = "error"
    )
  }, error = function(e) {
    warning(paste("parLapply timed out or failed:", e$message))
    return(NULL)
  })
...
cl <- makeCluster(config$n_cores, type = "PSOCK",
                  outfile = file.path(config$results_dir, "cluster.log"))
...
p <- ggplot(rm_final, aes(x = obs_mean, y = sim_mean)) +
  ...
  labs(
    title    = "DayCent RM-Level Canola Calibration: Simulated vs Observed",
