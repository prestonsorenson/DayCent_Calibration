# =============================================================================
# DayCent SW0 Spring Wheat Calibration — RM Level
...
config <- list(
  ...
  template_dir   = "/home/prs352/Daycent/Crop_Optimization/Models/Wheat/Copy_Folder",
  sites_dir      = "/home/prs352/Daycent/Crop_Optimization/Models/Wheat/qrts",
  results_dir    = "/home/prs352/Daycent/Crop_Optimization/Models/Wheat/results",
  obs_data_path  = "/home/prs352/Daycent/Crop_Optimization/Models/input_data/wheat_c_gmsq.csv",
...
obs_rm_mean <- obs_all %>%
  group_by(RMNO) %>%
  summarise(obs_mean_grain_c = mean(cgrain, na.rm = TRUE), .groups = "drop")
...
objective_fn <- function(param_vec) {
  ...
  joined <- simulated %>%
    inner_join(obs_all %>% rename(obs_grain_c = cgrain),
               by = c("RMNO", "Year"))
...
final_joined <- final_sim %>%
  inner_join(obs_all %>% rename(obs_grain_c = cgrain), by = c("RMNO", "Year"))
...
p <- ggplot(rm_final, aes(x = obs_mean, y = sim_mean)) +
  ...
  labs(
    title    = "DayCent RM-Level Wheat Calibration: Simulated vs Observed",
